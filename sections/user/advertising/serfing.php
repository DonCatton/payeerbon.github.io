...
<div class="alls_block_check_ser">
	...
	if (empty($checksbans['ban'])) {
		...
		$checkmysqlin = $db->row("serf","type = 1 ORDER BY sum DESC");
		$infscheck = checkser(1);

		if ($infscheck > 0) {
			foreach ($checkmysqlin as $serflist) {
								...
								if ($checkmysqlins['time'] < time() AND $checkmysqlins['status'] != '1') {
									echo '
										...
										<div class="serfer">
											...
											<div class="text_ser">
												'.htmlspecialchars($serflist['pod_text']).'
											</div>
			...
			echo '<span style="font-size: 13px;text-align: center;
	    display: block;">В данный момент сайтов для серфинга нет</span>';
		}
	} else {
		baninfo();
	}

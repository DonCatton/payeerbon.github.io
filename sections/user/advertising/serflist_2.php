<?top('Список баннерных серфингов');?>
<link rel="stylesheet" href="/system/mane/css/serferc.css">
<div class="alert alert-info">
	<strong>Баннерный Серфинг</strong>
	<br>
	Уникальные возможности по увеличению посещаемости вашего сайта. Тонкая настройка распределения посещений в течении дня. Возможность прикрепить свой баннер к серфингу.
</div>

<a href="/addserf_2" class="add_advert">Добавить<br> баннер</a>
<div class="alls_blocks_list_serf">
<?
$limssss = '7';

$Param1 = "SELECT * FROM `serf` WHERE users = '".clear(ULG)."' AND type = '2' ORDER BY `id` DESC LIMIT 0, ".$limssss;
$Param2 = "SELECT * FROM `serf` WHERE users = '".clear(ULG)."' AND type = '2' ORDER BY `id` DESC LIMIT START, ".$limssss;
$Param4 = 'serflist_2';


$Count = $db->column("serf","users = ? && type = 2",[1=>ULG]);
if (!$_GET['page']) {
	$_GET['page'] = 1;
...
if ($checkon2 > 0) {
						...
						<tr>
							...
							<td width="406">
								...
								<div class="po_headers_info_serf">
									<span><img class="img_blocks_inf_serf" src="'.$infserf['web_banner'].'" alt=""></span>
								</div>
	...
	echo '<span class="info_check_er">У вас нет сайтов, <a href="/addserf_2">добавить сайт</a></span>';
}
PageSelector($Param4, $_GET['page'], $Count, $limssss, 'serflist_2?page=');	// Вывод страниц
//
?>
</div>
		...
		<div class="mobal_degi">
			<div class="content_vibor_in">
				...
				<div class="body_content_mob ">
					...
					<input type="hidden" id="id_users_l_v" value="<?=$_SESSION['login']?>">

...
$Param1 = "SELECT * FROM `serf` WHERE type = 2 ORDER BY `id` DESC LIMIT 0, ".$lims;
$Param2 = "SELECT * FROM `serf` WHERE type = 2 ORDER BY `id` DESC LIMIT START, ".$lims;
$Param4 = 'admins';

$Count = $db->column("serf","type = 2",[]);
$sum1 = $db->selects("serf","type = 2",[],"SUM(sum) as s, SUM(balance) as sb");
?>
...
if (!empty($Result)) {
	foreach ($Result as $list_bonus) {
		...
		echo ' 
					<tr>
						...
						<td>
							<a href="'.$list_bonus['web'].'" target="_blank">'.$list_bonus['web'].' <a href="https://www.virustotal.com/ru/url/submission/?force=1&url='.$list_bonus['web'].'" target="_blank"><button title="Проверить на вирусы" class="btn btn-primary btn-xs dropdown-toggle tooltips" data-container="body" data-original-title="Проверить" type="button" id="dropdownMenu_76710" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><i class="fa fa-shield" aria-hidden="true"></i></button></a>
							<br>
							<div><img src="'.$list_bonus['web_banner'].'" style="width: 100%"></div>
						</td>
	...
	echo '<tr><td colspan="7">Список баннерного серфинга чист</td></tr>';
}
?>
				</tbody>
